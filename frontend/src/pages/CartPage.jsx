...
import { Box, Button, Center, VStack, SimpleGrid, Image, Text, useToast, Spinner, useColorModeValue, Tooltip, Input } from '@chakra-ui/react';
...
const CartPage = () => {
  ...
  const handleApplyCoupon = async () => {
    try {
      ...
      const response = await applyCoupon(currentUser._id, couponCode);
      if (response.success) {
  ...
  const handleRemoveCoupon = async () => {
    try {
      ...
      const response = await removeCoupon(currentUser._id, couponCode);
      if (response.success) {
        setCouponCode('');
